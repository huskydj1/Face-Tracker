...
'''
# When Using Serengil's Tensorflow Re-Implementation
...
def interpretRetina(conf_threshes, iou_threshes, manual_conf = 0.5):
            ...
            for input_short, input_name in input_videos.items(): # For each input video
                ...
                for frame_num in range(int(cap.get(cv2.CAP_PROP_FRAME_COUNT))):
                    ...
                    if ret:
                        ...
                        if "walking-and-pause" in input_name or "dog" in input_name:
                            drawframe.notate(img=frame, boxes = boxes[probs>=manual_conf], probs = probs[probs>=manual_conf])
                            ...
                            drawframe.notate(img=frame, boxes = boxes[probs>=manual_conf], probs = probs[probs>=manual_conf], thickness = 4, fontScale = 1.0, fontColor = (240, 125, 2))
